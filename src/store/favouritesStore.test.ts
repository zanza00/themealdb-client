...
describe("useFavouritesStore", () => {
  ...
  test("initial state should have no favourites", () => {
    const favourites = useFavouritesStore.getState().favourites;
    expect(favourites).toEqual([]);
    const count = useFavouritesStore.getState().getFavouritesCount();
    expect(count).toBe(0);
  });
  ...
  test("addFavourite should not add a duplicate meal", () => {
  ...
  test("isFavourite should return true for a favourite meal and false otherwise", () => {
    const { addFavourite, isFavourite } = useFavouritesStore.getState();
    expect(isFavourite(mockMeal.idMeal)).toBe(false); // Initially not a favourite
    addFavourite(mockMeal);
    expect(isFavourite(mockMeal.idMeal)).toBe(true); // Should be a favourite after adding
  });

  test("removeFavourite should remove a meal from favourites", () => {
    const {
      addFavourite,
      removeFavourite,
      favourites,
    ...
    addFavourite(mockMeal);
    expect(getFavouritesCount()).toBe(1);
    expect(isFavourite(mockMeal.idMeal)).toBe(true);

    removeFavourite(mockMeal.idMeal);
    expect(getFavouritesCount()).toBe(0);
    expect(isFavourite(mockMeal.idMeal)).toBe(false);
    expect(favourites).not.toContainEqual(mockMeal);
  });

  test("removeFavourite should do nothing if meal is not a favourite", () => {
    const { removeFavourite, getFavouritesCount } =
      useFavouritesStore.getState();
    removeFavourite("nonExistentId");
    expect(getFavouritesCount()).toBe(0);
  });

  test("getFavouritesCount should return the correct number of favourites", () => {
    const { addFavourite, getFavouritesCount } = useFavouritesStore.getState();
